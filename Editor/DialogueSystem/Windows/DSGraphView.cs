...
//SUMMARY//
//Este script se encarga de generar el DialogGraph y de implementar sus funciones como crear nodos y grupos, eliminarlos y agruparlos
namespace DS.Windows
{
    ...
    public class DSGraphView : GraphView
    {
        ...
        private void AddManipulators()
        {
            ...
            this.AddManipulator(CreateNodeContextualMenu("Add Node (Master)",DSDialogueType.Master));
            this.AddManipulator(CreateNodeContextualMenu("Add Node (Single Choice)", DSDialogueType.SingleChoice));
            this.AddManipulator(CreateNodeContextualMenu("Add Node (Multiple Choice)", DSDialogueType.MultipleChoice));
            this.AddManipulator(CreateNodeContextualMenu("Add Node (Response)",DSDialogueType.Response));
            this.AddManipulator(CreateNodeContextualMenu("Add Node (Party)",DSDialogueType.Party));
            this.AddManipulator(CreateNodeContextualMenu("Add Node (Position)",DSDialogueType.Position));
        ...
        public DSNode CreateNode(string nodeName, DSDialogueType dialogueType, Vector2 position, bool shouldDraw = true)
        {
            switch(dialogueType){
                default:
                    DSNode node0 = new DSNode();
                    return node0;
                case DSDialogueType.SingleChoice:
                    DSSingleChoiceNode node1 = new DSSingleChoiceNode();
                    node1.Initialize(nodeName, this, position);
                    if (shouldDraw){node1.Draw();}
                    AddUngroupedNode(node1);
                    return node1;
                case DSDialogueType.MultipleChoice:
                    DSMultipleChoiceNode node2 = new DSMultipleChoiceNode();
                    node2.Initialize(nodeName, this, position);
                    if (shouldDraw){node2.Draw();}
                    AddUngroupedNode(node2);
                    //Rellenar nodo
                    return node2;
                case DSDialogueType.Response:
                    DSResponseNode node3 = new DSResponseNode();
                    node3.Initialize(nodeName, this, position);
                    if (shouldDraw){node3.Draw();}
                    AddUngroupedNode(node3);
                    //Rellenar nodo
                    return node3;
                case DSDialogueType.Party:
                    DSPartyNode node4 = new DSPartyNode();
                    node4.Initialize(nodeName, this, position);
                    if (shouldDraw){node4.Draw();}
                    AddUngroupedNode(node4);
                    //Rellenar nodo
                    return node4;
                case DSDialogueType.Position:
                    DSPositionNode node5 = new DSPositionNode();
                    node5.Initialize(nodeName, this, position);
                    if (shouldDraw){node5.Draw();}
                    AddUngroupedNode(node5);
                    //Rellenar nodo
                    return node5;
                case DSDialogueType.Master:
                    DSMasterNode node6 = new DSMasterNode();
                    node6.Initialize(nodeName, this, position);
                    if (shouldDraw){node6.Draw();}
                    AddUngroupedNode(node6);
                    //Rellenar nodo
                    return node6;
            }
        }
